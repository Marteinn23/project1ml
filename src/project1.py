...
def main():
    ...
    X_train_constrained, X_test_constrained, y_train_constrained, y_test_constrained = (
        train_test_split(
            X_constrained,
            y,
            test_size=0.2,
    ...
    estimator = decision_tree_with_gridsearch(
        X_train_constrained, y_train_constrained, False
    )
    pred = estimator.predict(
        preprocess_single_url(
            "https://www.sciencedirect.com/science/article/pii/S0167404823004558?via%3Dihub#se0250"
        )
    )
    ...
    best_models["rf"] = random_forest_gridsearch(
        X_train_constrained, y_train_constrained, X_test_constrained, y_test_constrained
    )

    print("Neural Network grid search? ")
    best_models["nn"] = neural_network_gridsearch(
        X_train_constrained, y_train_constrained, X_test_constrained, y_test_constrained
    )

    print("Naive Bayes grid search? ")
    best_models["nb"] = naive_bayes_gridsearch(
        X_train_constrained, y_train_constrained, X_test_constrained, y_test_constrained
    )

    # Voting classifier with best models
    print("Voting classifier grid search? ")
    final_classifier = voting_classifier_gridsearch(
        X_train_constrained,
        y_train_constrained,
        X_test_constrained,
        y_test_constrained,
...
def voting_classifier_gridsearch(X_train, y_train, X_test, y_test, best_models):
    ...
    grid = GridSearchCV(
        ensemble, param_grid=param_grid, scoring=fn_scorer, cv=5, n_jobs=-1, verbose=1
    )
...
def fn_focused_scorer(y_true, y_pred, fn_weight=5.0, fp_weight=1.0):
    ...
    weighted_score = (fn_weight * recall + fp_weight * specificity) / (
        fn_weight + fp_weight
    )
